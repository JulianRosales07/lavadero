...
// Recharts se carga solo cuando de verdad hay una gráfica que pintar.
const chartFallback = () => <Skeleton className="h-64 w-full sm:h-72" />;
...
const TopServicesChart = dynamic(
  () => import('@/components/reports/charts').then((mod) => mod.TopServicesChart),
  { ssr: false, loading: chartFallback },
);

const PaymentMethodsChart = dynamic(
  () => import('@/components/reports/charts').then((mod) => mod.PaymentMethodsChart),
  { ssr: false, loading: chartFallback },
);
